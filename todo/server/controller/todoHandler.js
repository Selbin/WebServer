...
const createTodo = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(createError(500, 'todo creation failed'))
  }
}
...
const showAllTodo = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount > 0) res.status(200).send(result.rows)
    else res.status(200).send({ message: 'todo is empty' })
  } catch (e) {
    res.status(500).send(createError(500, 'fetch failed'))
  }
}
...
const deleteTodo = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount > 0) res.status(200).send(result.rows[0])
    else res.status(404).send(createError(404, 'todo not found'))
  } catch (e) {
    res.status(500).send(createError(500, 'todo deletion failed'))
  }
}
...
const updateTodo = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(createError(500, 'updation failed'))
  }
}
